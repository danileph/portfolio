...
export const projectsMap = async (data: (PageObjectResponse | PartialPageObjectResponse)[]) => {
  ...
  data.forEach((project, i) => {
    ...
    const mappedTechnologies: IProject = {
      ...
      description: 'Description' in properties ? properties['Description'].rich_text.map(item => item.plain_text).join('&#10;') : null,
      type: 'Type' in properties ? properties['Type'].select?.name : null,
      myTechnologies: 'My technologies' in properties && properties['My technologies'].relation.length !== 0 ? technologies.filter(item => properties['My technologies'].relation.find(rel => item.id === rel.id)) : null,
      otherTechnologies: 'Other technologies' in properties && properties['Other technologies'].relation.length !== 0 ? technologies.filter(item => properties['Other technologies'].relation.find(rel => item.id === rel.id)) : null,
